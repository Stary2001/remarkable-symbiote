...
auto Stroke::getPoint(int index) const -> Point {
    if (index < 0 || index >= this->points.size()) {
        printf("Stroke::getPoint(%i) out of bounds!", index);
...
void Stroke::setPressure(const vector<double>& pressure) {
    ...
    if (this->points.size() - 1 != pressure.size()) {
        printf("invalid pressure point count: %s, expected %s", std::to_string(pressure.size()).data(),
...
void Stroke::debugPrint() {
    printf("Stroke %p / hasPressure() = %i ", this, this->hasPressure());
